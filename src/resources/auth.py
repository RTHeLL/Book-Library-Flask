...
class AuthRegister(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            db.session.rollback()
            return {'message': 'User with this Login or E-Mail exists'}, 409
...
class AuthLogin(Resource):
    def get(self):
        ...
        if not user or not check_password_hash(user.password, auth.get('password', '')):
            return "", 401, {"WWW-Authenticate": "Basic realm='No found user or wrong data'"}
        token = jwt.encode(
            {
                "user_id": user.uid,
...
def token_required(func):
    ...
    def wrapper(self, *args, **kwargs):
        ...
        if not token:
            return "", 401, {"WWW-Authenticate": "Basic realm='Authentication required'"}
        try:
            uid = jwt.decode(token, app.config['SECRET_KEY'], algorithms='HS256')['user_id']
